...
Pass 4: volume (pre-gain, when clamped) → alimiter (Volumax, peak reduction) → loudnorm (linear mode, input stats from Pass 3) → aresample (source rate) → adeclick → alimiter (final-stage brickwall, source rate) → astats → aspectralstats → ebur128
```
